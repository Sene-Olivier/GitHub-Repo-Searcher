...
function App() {
  ...
  const handleSubmitSearchForm = (textToSearch: string) => {
    // A la soumission du formulaire, je mets à jour la valeur de searchText
    setSearchText(textToSearch);
    setPageNumber(1);
  };

  useEffect(() => {
    // Je lance la recherche seulement si j'ai un texte à rechercher
    if (searchText !== '') {
      // Je vais faire une requête à l'API
      // Axios me permet de passer un deuxième paramètre à ma requête
      // Ce paramètre me permet de passer les paramètres d'URL (Query Params) de ma requête
      axios
        ...
        .then((response) => {
          ...
          // Les données retournées par l'API sont dans response.data (pas besoin de faire un response.json())
          // Je modifie les données stockées dans mes states avec ceux provenant de l'API
          if (pageNumber === 1) {
      ...
      // Sinon je mets à jour mes states avec des valeurs vides
      setRepos([]);
      setTotalCount(0);
    }
